...
def get_delta_f(base_directory):

    # Load F Matrix
    f_matrix = np.load(os.path.join(base_directory, "F_Matrix.npy"))
    print("f_matrix", np.shape(f_matrix))

    # Get Smoothed
    #f_matrix = moving_average(f_matrix, window_size=2)
    ...
    #f_zero = moving_average(f_matrix, window_size=1800)
    ...
    # Save Matrix
    np.save(os.path.join(base_directory, "df_over_f_matrix.npy"), df_matrix)
    ...
    forceAspect(plt.gca())
    plt.savefig(os.path.join(base_directory, "df_over_f_rastermap.png"))
    #plt.show()
    plt.close()