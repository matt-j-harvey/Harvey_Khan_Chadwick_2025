...
def plot_trajectories(data_directory, session, output_directory):
    ...
    vis_context_vis_1_tensor = vis_context_vis_1_tensor[:, start_window:stop_window]
    vis_context_vis_2_tensor = vis_context_vis_2_tensor[:, start_window:stop_window]
    odr_context_vis_1_tensor = odr_context_vis_1_tensor[:, start_window:stop_window]
    odr_context_vis_2_tensor = odr_context_vis_2_tensor[:, start_window:stop_window]
    ...
    context_cd = context_cd / np.linalg.norm(context_cd)
    """
    ...
    """
    visual_stim_difference = np.subtract(mean_vis_context_vis_1, mean_vis_context_vis_2)
    visual_stim_cd = np.mean(visual_stim_difference[preceeding_window:preceeding_window + 6], axis=0)
    ...
    visual_stim_cd = np.load(os.path.join(lick_cd_folder, session, "Visual Context Lick CD", "vis_context_lick_cd.npy"))
    ...
    # Get Projections
    vis_context_vis_1_lick_projection = np.dot(mean_vis_context_vis_1, visual_stim_cd)
    vis_context_vis_2_lick_projection = np.dot(mean_vis_context_vis_2, visual_stim_cd)
    odr_context_vis_1_lick_projection = np.dot(mean_odr_context_vis_1, visual_stim_cd)
    odr_context_vis_2_lick_projection = np.dot(mean_odr_context_vis_2, visual_stim_cd)
    ...
    axis_1.scatter([vis_context_vis_1_lick_projection[preceeding_window]], [vis_context_vis_1_context_projection[preceeding_window]], c='k', marker='8')
    ...
    axis_1.set_xlabel("Stimulus Coding Dimension")
    axis_1.set_ylabel("Context Coding Dimension")

    # Plot Vector Field
    plot_vector_field(axis_1, visual_stim_cd, context_cd, recurrent_weights, [lick_min, lick_max], [context_min, context_max])
