...
import local_nmf_base
import local_nmf_factor
...
class BaseNMF(local_nmf_base.VideoFactorizer):
    ...
    def __init__(self,
        ...
        self.spatial = local_nmf_factor.TensorFactor((self.num_components, video_shape[0]),
                                    scratch=True,
                                    device=self.device,
                                    dtype=torch.float32)
        self.intermediate = local_nmf_factor.TensorFactor((video_shape[1], self.num_components),
                                         device=self.device,
                                         dtype=torch.float32)
        self.temporal = local_nmf_factor.TensorFactor((self.num_components, video_shape[2]),
                                     scratch=True,
                                     device=self.device,
                                     dtype=torch.float32)
        self.covariance = local_nmf_factor.TensorFactor((self.num_components,)*2,
                                       device=self.device,
                                       dtype=torch.float32)
        self.scale = local_nmf_factor.TensorFactor((self.num_components,),
                                  scratch=True,
                                  device=self.device,
                                  dtype=torch.float32)
        self.index = local_nmf_factor.TensorFactor((self.num_components,),
                                  device=self.device,
                                  dtype=torch.long)
...
class LocalizedNMF(SpatialHals, TemporalHals, BaseNMF):
    ...
    def __init__(self,
        ...
        self.distance = local_nmf_factor.TensorFactor((self.num_components, video_shape[0]),
                                     scratch=True,
                                     device=self.device,
                                     dtype=torch.float32)
        self.regions = local_nmf_factor.TensorFactor((self.num_components,),
                                    device=self.device,
                                    dtype=torch.long)
        self.lambdas = local_nmf_factor.TensorFactor((self.num_components,),
                                    device=self.device,
                                    dtype=torch.float32)
