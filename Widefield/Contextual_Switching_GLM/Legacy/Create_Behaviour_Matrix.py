...
def create_behaviour_matrix(data_root_directory, base_directory, mvar_output_directory, lick_lag=1.0):
    ...
    # Load Downsampled AI
    downsampled_ai_file = os.path.join(mvar_output_directory, base_directory, "Downsampled_AI_Matrix_Framewise.npy")
    downsampled_ai_matrix = np.load(downsampled_ai_file)

    # Create Stimuli Dictionary
    stimuli_dictionary = GLM_Utils.load_rig_1_channel_dict()
    ...
    n_lags = int(np.around(1000/36, 0))
    print("n_lags", n_lags)

    lick_threshold = np.load(os.path.join(data_root_directory, base_directory, "Behaviour", "Downsampled_Lick_Threshold.npy"))
