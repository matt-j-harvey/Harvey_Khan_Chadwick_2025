...
def extract_onsets(base_directory, ai_filename, save_directory, lick_threshold=0.13, visualise_lick_threshold=True):
    ...
    """
    plt.plot(opto_trace, c='m')
    plt.scatter(opto_onsets, np.ones(len(opto_onsets)))
    plt.plot(combined_visual_trace, c='b')
...
def check_if_trial_is_opto(trial_type, trial_onset, opto_onset_list, irrel_onset, acceptable_window=20):

    if trial_type == 1 or trial_type == 2:
        vis_onset = trial_onset

    elif trial_type == 3 or trial_type == 4:
        if irrel_onset == None:
            vis_onset = trial_onset
        else:
            vis_onset = irrel_onset

    trial_is_opto = False
    for opto_onset in opto_onset_list:
        if vis_onset > (opto_onset - acceptable_window) and vis_onset < (opto_onset + acceptable_window):
            trial_is_opto = True

    return trial_is_opto


def classify_trial(onset, onsets_dictionary, traces_dictionary, opto_pattern_labels, trial_index, opto_stim_index, lick_threshold, opto_preceeding_window):
    ...
    22 Opto Trial - Boolen Yes or No
    22 Opto Pattern - Which Opto Stimuli Was Presented
    """
    ...
    # Get Opto Onset
    opto_type = check_if_trial_is_opto(trial_type, onset, onsets_dictionary["opto_onsets"], irrel_onset, acceptable_window=opto_preceeding_window)

    if opto_type == 1:
        opto_pattern = opto_pattern_labels[opto_stim_index]
    else:
        opto_pattern = None
...
def print_behaviour_matrix(behaviour_matrix):
    ...
    for t in behaviour_matrix:
        print("Trial:" ,t[0]," Type:",t[1]," Lick:",t[2]," Correct:",t[3]," Rewarded:",t[4]," Irrel_Preceed:",t[5]," Irrel Type:",t[6]," Ignore Irrel:",t[7],"Block Number:",t[8],"First In Block:",t[9],"In Stable Window:",t[10],"Onset:",t[11]," Offset:",t[12]," Photodiode_Onset:",t[16]," Photodiode_Offset:",t[17], "Opto_Type", t[22])
...
def create_behaviour_matrix(base_directory, opto_preceeding_window=3000, behaviour_only=False):
    ...
    # Load Opto Pattern Labels
    opto_pattern_labels = np.load(os.path.join(base_directory, "Stimuli_Onsets", "Opto_Pattern_Labels.npy"))
    ...
    for trial in trial_onsets:
        trial_vector = classify_trial(trial, onsets_dictionary, traces_dictionary, opto_pattern_labels, trial_index, opto_stim_index, lick_threshold, opto_preceeding_window)
        trial_matrix.append(trial_vector)

        trial_index += 1
        opto_stim_index += trial_vector[-2]
