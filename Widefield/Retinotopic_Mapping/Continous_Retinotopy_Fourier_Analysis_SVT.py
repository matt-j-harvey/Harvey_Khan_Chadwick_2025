...
def get_average_movie(base_directory, stimulus_type, indicies, image_height, image_width):

    # Get Number Of Trials
    mean_movie = np.load(os.path.join(base_directory, "Stimuli_Evoked_Responses", stimulus_type + "_Sweep", stimulus_type + "_Sweep_Activity_Matrix_Average.npy"))
...
def plot_maps(base_directory, horizontal_power_map, horizontal_phase_map, vertical_power_map, vertical_phase_map, sign_map):
    ...
    save_directory = os.path.join(base_directory, "Maps")
    ...
    plt.savefig(os.path.join(save_directory, "horizontal_power_map.png"))
    plt.close()
...
def perform_fourier_analysis(base_directory):
    ...
    # Load Mask
    indicies, image_height, image_width = Retinotopy_Utils.load_generous_mask(base_directory)

    # Get Average Movies
    horizontal_average_movie = get_average_movie(base_directory, "Horizontal", indicies, image_height, image_width)
    vertical_average_movie = get_average_movie(base_directory, "Vertical", indicies, image_height, image_width)

    # Perform Fourier Mapping
    print("Horiztonal Average Movie Shape", np.shape(horizontal_average_movie))
    number_of_cycles = np.shape(horizontal_average_movie)[0] - 1
    ...
    example_image = np.load(os.path.join(base_directory, "Preprocessed_Data", "Frame_Mean.npy"))[0]
    print("EXample Image Shape", np.shape(example_image))
    ...
    plt.imshow(thresholded_sign_map)
    plt.show()
    threshold = 0.1
