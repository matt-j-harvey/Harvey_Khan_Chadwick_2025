...
def adaptive_fit(video_mats,
    ...
    region_metadata = local_nmf_video.RegionMetadata(region_mats[0].shape[0],
    ...
    low_rank_video = local_nmf_video.LowRankVideo(
...
def extract_region_metadata(valid_mask,
    ...
    print("keep mask", keep_mask.dtype)
...
def factor_region_videos(video_mats,
    ...
    region_low_rank_videos = [
        local_nmf_video.LowRankVideo((int(np.sum(mask)), max_rank, video_mats[1].shape[-1]),
...
def rank_linesearch(low_rank_video,
    ...
    for video in region_lr_videos:
        ...
        region_factors.append(local_nmf_demix.HalsNMF(max_ranks[-1],
                                      (video.shape[0],
                                       len(video),
                                       video.shape[-1]),
                                      device=device))
    ...
    nmf_factors = local_nmf_demix.LocalizedNMF(np.sum(max_ranks),
