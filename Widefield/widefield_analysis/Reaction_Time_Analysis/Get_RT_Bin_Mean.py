...
def get_rt_bin_means(data_root, session_list, n_bins, bin_start_list, bin_stop_list, start_window, stop_window, output_directory):
    ...
    for bin_index in tqdm(range(n_bins)):
        ...
        for mouse in tqdm(session_list):
            ...
            for session in mouse:
                ...
                session_directory = os.path.join(data_root, session)
                print("session directory", session_directory)

                # Create Hit RT Matrix
                hit_rt_time_matrix = Create_Hit_RT_Matrix.create_hit_rt_matrix(session_directory)

                # Get Hit Tensor
                session_mean = get_session_mean(session_directory, hit_rt_time_matrix, start_window, stop_window, bin_start, bin_stop)
                print("session_mean", np.shape(session_mean))
