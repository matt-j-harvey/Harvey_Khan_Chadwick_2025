...
def get_common_length(design_matrix):
    ...
    length_list = []
    for item in design_matrix:
        item_length = np.shape(item)[0]
    ...
    for item in design_matrix:
        trimmed_item = item[0:min_length]
        trimmed_design_matrix.append(trimmed_item)

    for item in trimmed_design_matrix:
        print(len(item))
...
def create_behaviour_matrix(data_root_directory, base_directory, mvar_output_directory, lick_lag=1.0):
    ...
    face_motion_svd = np.load(os.path.join(data_root_directory, base_directory, "Mousecam_Analysis", "Full_Face_Motion_SVD.npy"))
