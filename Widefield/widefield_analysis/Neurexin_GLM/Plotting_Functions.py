...
def visualise_mean_regressor(mean_regressor, save_directory, x_values, magnitude=[-1,1], cmap=GLM_Utils.get_musall_cmap()):
    ...
    for timepoint_index in range(n_timepoints):
        ...
        # Plot Data
        im = axis_1.imshow(timepoint_data, vmin=magnitude[0], vmax=magnitude[1], cmap=cmap)
...
def plot_roi_by_rt_window(session_list, output_root, start_window, stop_window, rt_window_start_list, rt_window_stop_list, roi_list):
    ...
    for rt_window_index in range(n_rt_windows):
        ...
        if len(group_list_real) > 0:
            ...
            # Get Mean and SEM
            real_mean, real_lower, real_upper = GLM_Utils.get_mean_sd(group_list_real)
            predicted_mean, predicted_lower, predicted_upper = GLM_Utils.get_mean_sd(group_list_predicted)
            mousecam_mean, mousecam_lower, mousecam_upper = GLM_Utils.get_mean_sd(group_list_mousecam)
...
def compare_roi_across_groups(regressor_1, regressor_2, save_directory, start_window, stop_window, roi_list, graph_name):
    ...
    regressor_1 = np.nan_to_num(regressor_1)
    regressor_2 = np.nan_to_num(regressor_2)
    ...
    axis_1.plot(x_values, regressor_2_mean, c=neurexin_pink)
    axis_1.fill_between(x=x_values, y1=regressor_2_lower_bound, y2=regressor_2_upper_bound, alpha=0.5, color=neurexin_pink)
...
def compare_roi_within_groups(regressor_1, regressor_2, save_directory, start_window, stop_window, roi_list, graph_name, ylim=None):
    ...
    if ylim != None:
        axis_1.set_ylim([ylim[0], ylim[1]])

    axis_1.plot(x_values, regressor_1_mean, c='tab:orange')
    axis_1.fill_between(x=x_values, y1=regressor_1_lower_bound, y2=regressor_1_upper_bound, alpha=0.5, color="tab:orange")

    axis_1.plot(x_values, regressor_2_mean, c='royalblue')
    axis_1.fill_between(x=x_values, y1=regressor_2_lower_bound, y2=regressor_2_upper_bound, alpha=0.5, color="royalblue")
