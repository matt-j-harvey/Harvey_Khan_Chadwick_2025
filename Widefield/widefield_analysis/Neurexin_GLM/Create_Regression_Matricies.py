...
def create_regression_matricies(data_directory, session, mvar_directory_root, regressor_list, z_score, baseline_correct):

    # Load Activity Tensors
    vis_1_activity_tensor = open_tensor(os.path.join(mvar_directory_root, session, "Activity_Tensors", regressor_list[0]))
    vis_2_activity_tensor = open_tensor(os.path.join(mvar_directory_root, session, "Activity_Tensors", regressor_list[1]))
    print("vis_1_activity_tensor", np.shape(vis_1_activity_tensor))

    # Load Behaviour Tensors
    vis_1_behaviour_tensor = open_tensor(os.path.join(mvar_directory_root, session, "Behaviour_Tensors", regressor_list[0]))
    vis_2_behaviour_tensor = open_tensor(os.path.join(mvar_directory_root, session, "Behaviour_Tensors", regressor_list[1]))
    ...
    print("behaviour_regressor", np.shape(behaviour_regressor))
