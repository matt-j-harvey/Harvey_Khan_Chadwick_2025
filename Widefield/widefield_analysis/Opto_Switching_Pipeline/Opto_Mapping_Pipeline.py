...
def run_opto_mapping_pipeline(data_root, experiment, output_root, start_window, stop_window, regressor_list, comparison_window_start, comparison_window_stop):
    ...
    for session in tqdm(session_list, desc="Mouse"):
        ...
        Create_Tensors.create_activity_tensor(data_root, session, output_root, "visual_context_control_onsets.npy", start_window, stop_window, comparison_window_start, comparison_window_stop)
        Create_Tensors.create_activity_tensor(data_root, session, output_root, "odour_context_control_onsets.npy", start_window, stop_window, comparison_window_start, comparison_window_stop)
        Create_Tensors.create_activity_tensor(data_root, session, output_root, "visual_context_light_onsets.npy", start_window, stop_window, comparison_window_start, comparison_window_stop)
        Create_Tensors.create_activity_tensor(data_root, session, output_root, "odour_context_light_onsets.npy", start_window, stop_window, comparison_window_start, comparison_window_stop)

        # Create Behaviour Tensors
        Create_Tensors.create_behaviour_tensor(data_root, session, output_root, "visual_context_control_onsets.npy", start_window, stop_window, comparison_window_start, comparison_window_stop)
        Create_Tensors.create_behaviour_tensor(data_root, session, output_root, "odour_context_control_onsets.npy", start_window, stop_window, comparison_window_start, comparison_window_stop)
        Create_Tensors.create_behaviour_tensor(data_root, session, output_root, "visual_context_light_onsets.npy", start_window, stop_window, comparison_window_start, comparison_window_stop)
        Create_Tensors.create_behaviour_tensor(data_root, session, output_root, "odour_context_light_onsets.npy", start_window, stop_window, comparison_window_start, comparison_window_stop)

        # Create Regression Matricies
        design_matrix, delta_f_matrix = Create_Regression_Matricies_Mean.create_regression_matricies(data_root, session, output_root, z_score=False,  baseline_correct=False)
        ...
        # Run Regression
        Fit_Full_Model.fit_full_model(session, output_root, design_matrix, delta_f_matrix)

    # Get Group Average Coefs
    Get_Group_Average_Results.get_group_average_results(data_root, control_session_list, output_root, start_window, stop_window, comparison_window_start, comparison_window_stop, regressor_list, experiment[0] + "_Control")
    Get_Group_Average_Results.get_group_average_results(data_root, opto_session_list, output_root, start_window, stop_window, comparison_window_start, comparison_window_stop, regressor_list, experiment[0] + "_Opsin")
